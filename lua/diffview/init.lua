...
function M.init()
  ...
  DiffviewGlobal.emitter:on(
    "review_cleanup_completed",
    function(_)
      api.nvim_exec_autocmds(
        "User",
        { pattern = "DiffviewReviewCleanupCompleted", modeline = false }
      )
    end
  )
...
function M.show(args)
  local view = lib.diffview_show(args)
  if view then view:open() end
end
...
M.completers = {
  ...
  DiffviewOpenJson = function(ctx)
    if ctx.argidx == 2 then return vim.fn.getcompletion(ctx.arg_lead, "file", 0) end
    local has_rev_arg = false
    local adapter = M.get_adapter()
