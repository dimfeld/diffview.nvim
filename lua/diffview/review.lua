...
local function get_file_blob_hash(view, file_entry)
  if file_entry.is_json_description then return nil end
...
local function get_head_commit_hash(view)
  ...
  if head_rev and head_rev.commit then return head_rev.commit end
  return nil
end
...
function M.mark_all_reviewed(view)
  if not is_review_enabled() then return 0 end
  ...
  if not view.files then return 0 end
  ...
  if count > 0 then view.review_state.store:save_state(view.review_state) end
...
function M.get_file_status(view, file_entry)
  if not is_review_enabled() then return nil end

  if not view or not view.review_state then return nil end
  ...
  if view.get_cached_review_status then return view:get_cached_review_status(file_entry) end
...
---@return ReviewStore
function M.get_store() return review_store.get_store() end
...
function M.cleanup_stale_reviews(view)
  ...
  local store = review_store.get_store()
  local result =
    store:cleanup_stale_branches(view.adapter, { dry_run = false, max_age_days = max_age_days })
